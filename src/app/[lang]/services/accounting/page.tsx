...
import {
    Calculator, ArrowRight, BookOpen, Receipt, Building2,
    Shield, FileText, BarChart3, Search, CheckCircle2
} from "lucide-react";
import Link from "next/link";
import type { Metadata } from "next";

export const metadata: Metadata = {
    title: "บัญชีและภาษี | Valcot Partners",
    description: "บริการบัญชีและภาษีสำหรับธุรกิจในประเทศไทย ทำบัญชีรายเดือน ยื่นภาษี งบการเงินประจำปี โดยนักบัญชีที่ได้รับอนุญาต",
};
...
export default async function AccountingPage({
    ...
    const coverageItems = [
        { icon: BookOpen, text: dict.servicesPages.accounting.coverageItems[0] },
        { icon: Receipt, text: dict.servicesPages.accounting.coverageItems[1] },
        { icon: Calculator, text: dict.servicesPages.accounting.coverageItems[2] },
        { icon: Shield, text: dict.servicesPages.accounting.coverageItems[3] },
        { icon: FileText, text: dict.servicesPages.accounting.coverageItems[4] },
        { icon: Search, text: dict.servicesPages.accounting.coverageItems[5] },
    ];

    const highlights = dict.servicesPages.accounting.highlights;
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        <div className="max-w-2xl">
                            ...
                            <div className="mt-8 flex flex-col sm:flex-row gap-4">
                                ...
                                >
                                    {dict.servicesPages.accounting.getConsultation}
                        ...
                        <div className="relative hidden lg:block h-[500px]">
                            <div className="absolute inset-0 bg-gradient-to-br from-primary via-primary-dark to-[#0F172A] rounded-3xl shadow-2xl overflow-hidden shadow-primary/20">
                                ...
                                <div className="absolute -bottom-20 -right-20 w-80 h-80 bg-gold rounded-full filter blur-[100px] opacity-40"></div>
                                
                                {/* Floating Cards */}
                                <div className="absolute top-10 left-10 right-10 p-6 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 transform hover:-translate-y-1 transition-transform cursor-pointer">
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <span className="px-3 py-1 bg-green-500/20 text-green-300 text-xs font-semibold rounded-full border border-green-500/30">
                                            {lang === "th" ? "อัปเดตแล้ว" : "Updated"}
                                        </span>
                                    </div>
                                    <h3 className="text-white font-semibold mb-2">
                                        {lang === "th" ? "รายงานการเงินรายเดือน" : "Monthly Financial Report"}
                                    </h3>
                                    <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                                        <div className="w-full h-full bg-gradient-to-r from-gold to-white rounded-full"></div>
                                    </div>
                                </div>

                                <div className="absolute bottom-10 right-10 left-20 p-6 bg-white border border-border rounded-2xl shadow-xl transform hover:-translate-y-1 transition-transform cursor-pointer">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center">
                                            <Shield className="w-6 h-6 text-blue-600" />
                                        </div>
                                        <div>
                                            <p className="text-sm text-muted font-medium">{lang === "th" ? "การยื่นภาษีล่าสุด" : "Latest Tax Filing"}</p>
                                            <p className="font-bold text-primary">{lang === "th" ? "สำเร็จสมบูรณ์" : "Successfully Completed"}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        {coverageItems.map((item, idx) => (
                            ...
                            >
                                ...
                                <p className="text-muted text-sm leading-relaxed">
                                    {dict.servicesPages.accounting.coverageDesc}
                                </p>
                            </div>
                        ))}
                    </div>
                ...
                <div className="container-lg relative z-10">
                    <div className="max-w-4xl mx-auto bg-white/10 backdrop-blur-xl border border-white/20 p-10 sm:p-14 rounded-3xl text-center shadow-2xl">
                        <h2 className="text-3xl sm:text-4xl font-bold font-heading text-white leading-tight mb-6">
                            {dict.servicesPages.accounting.ctaTitle}
                        </h2>
                        <p className="text-lg text-white/80 mb-10 max-w-2xl mx-auto">
                            {dict.servicesPages.accounting.ctaSubtitle}
                        </p>
                        ...
                        <div className="flex flex-col sm:flex-row justify-center gap-4">
                            ...
                            >
                                {dict.servicesPages.accounting.ctaButton}
                                <ArrowRight className="w-5 h-5" />
                            </Link>
                        </div>
                        ...
                        <div className="mt-8 flex items-center justify-center gap-6 text-white/60 text-sm font-medium">
                            <div className="flex items-center gap-2">
                                ...
                                <span>{dict.servicesPages.accounting.check1}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <CheckCircle2 className="w-4 h-4 text-gold" />
                                <span>{dict.servicesPages.accounting.check2}</span>
                            </div>
                        </div>
                    </div>
                </div>
